...
namespace VietausWebAPI.Core.Application.Features.PurchaseFeatures.ServiceContracts
{
    ...
    {
        Task<OperationResult<Guid>> CreateAsync (PostPurchaseOrder postPurchaseOrder, CancellationToken ct = default);
        ...
        Task<byte[]> ExportPurchaseOrdersToExcelAsync(PurchaseOrderQuery query, CancellationToken ct = default);
        ...
        Task<OperationResult> CompleteAsync(Guid purchaseOrderId, CancellationToken ct = default);
    }
}
