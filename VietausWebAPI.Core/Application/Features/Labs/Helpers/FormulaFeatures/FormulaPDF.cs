    ...
    public class FormulaPDF : IFormulaPDF
    {
        ...
        private void BuildContent(IContainer c, ManufacturingVUPDF d, bool templateOnly)
        {
            ...
            c.Column(col =>
            {
                ...
                col.Item().PaddingTop(5).PaddingBottom(5).Table(table =>
                {
                    ...
                    // ====== GRID 3 cột (FontSize 8) + value đậm + thêm Khối lượng/mẻ ======
        ...
        private static void BuildNoteRequirementBox(IContainer c, string? note, string? requirement)
        {
            c.Table(t =>
            {
                t.ColumnsDefinition(cols =>
                {
                    cols.RelativeColumn(1);  // Note
                    cols.RelativeColumn(1);  // Requirement
                });
        ...
        private static void SignatureSection(IContainer c)
        {
            c.Border(1).BorderColor(Colors.Black).Padding(0).Column(col =>
            {
                ...
                col.Item().Row(r =>
                {
                    ...
                    r.RelativeItem().Element(HeaderBox).Text("Check by (PL&PU)").SemiBold();
